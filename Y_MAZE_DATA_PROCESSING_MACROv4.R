...
for(a in 1:aa){
  ...
  for(b in 1:num.sheet){
    ...
    trialnum<-read_excel(files[a],sheet=b,range = cell_rows(1:1),col_names = FALSE)
    num.row<-as.numeric(as.character(trialnum[1,2]))-1
    
    
    meta.data<-read_excel(files[a],sheet=b,range = cell_rows(1:num.row),col_names = FALSE)
   ...
   # subject.row<-match("<User-defined 1>",as.character(meta.data$...1))
    subject.row <- 35L
    ...
    dat<-read_excel(files[a],sheet=b,skip = num.row-1)
    ...
    coldat<-colnames(data.frame(read_excel(files[a],sheet=b,skip = num.row-1)))
    ...
    A.Arm.Entry<-table(Sequence)[1]
    B.Arm.Entry<-table(Sequence)[2]
    C.Arm.Entry<-table(Sequence)[3]
    A.Arm.Time<-time1
    B.Arm.Time<-time2
    C.Arm.Time<-time3
